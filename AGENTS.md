...
- Catalog item vectors come from legacy CSV `embedding` values generated by
  `text-embedding-3-small`; do not regenerate item vectors from `embedding_text`.
- User query vectors must use the same compatible catalog embedding model and
  no search-document/search-query prefixes by default.
...
- Assistant routing is a bounded LangGraph chat runtime: planner output,
  backend-gated tool validation, approved backend tools and response
  composition. The LLM may propose tool calls, but backend validation is the
  authorization boundary. No arbitrary tools or hidden server memory for visible
  candidates.
- LLM assistance is allowed only through structured LangChain/LangGraph output
  behind validated DTOs. It must not generate supplier facts, execute tools
  directly, write SQL or replace catalog evidence.
