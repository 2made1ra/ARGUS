...
embedding_template_version  text       -- legacy_csv_embedding
...
CSV legacy `embedding` is the source vector for catalog indexing. User query
embeddings must be generated with a compatible model, currently
...
legacy_embedding_present    boolean
legacy_embedding_dim        integer
service_category            text        -- canonical service taxonomy for search filters
service_category_confidence float
service_category_source     text        -- deterministic | llm | manual
service_category_reason     text
category_enrichment_status  text        -- pending | enriched | failed | skipped
category_enrichment_error   text
category_enriched_at        timestamptz
category_enrichment_model   text
category_enrichment_prompt_version text
created_at                  timestamptz
...
Generated catalog item vectors are not produced by the backend for MVP. Indexing
reads the legacy vector from `price_import_rows.raw["embedding"]`, validates its
dimension, upserts it to Qdrant, and updates `catalog_index_status`.
...
```text
Название: {name}
Категория: {service_category or category}
Раздел: {section}
...
- Omit empty lines.
- Prefer canonical `service_category` over raw CSV `category`.
- Strip tariff patterns such as `(аренда за 1 день)` or
  `(цена за 1 порцию)` from `name` before embedding.
...
`embedding_text` is not the catalog item vector source while legacy CSV
embeddings are authoritative. Changing the catalog model/dimension requires a
compatible legacy CSV and reindexing Qdrant from those CSV vectors.
...
vector: legacy CSV embedding generated by text-embedding-3-small
